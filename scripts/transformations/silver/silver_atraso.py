...
def run():
    ...
    con.execute(f"""
        ...
        SELECT * EXCLUDE({', '.join(keys_to_clean.keys())}), {', '.join(sql_transform)}
    ...
    if qtd_duplicados > 0:
        print(f"⚠️ Detectados {qtd_duplicados:,} duplicados. Deduplicando...")
            ...
            SELECT * EXCLUDE(row_num) FROM (
                SELECT t.*, 
                       ROW_NUMBER() OVER(
                           ...
                           ORDER BY t.ingestion_ts DESC, t.dat_referencia DESC, t.num_cpf ASC
                       ) as row_num
    ...
    header = f"{'PAREAMENTO (CHAVE -> DESC)':<47} | {'STATUS':<6} | {'TRATADOS (%)':<18} | {'DADOS AUSENTES':<15} | {'SEM CORRESPONDENCIA':<18}\n"
    ...
    for key, desc in pair_map.items():
        ...
        tratados_str = f"{perc_tratamento:>12.1f}%    "
        log_content += f"{f'{key} -> {desc}':<47} | {status:<6} | {tratados_str:<18} | {dados_ausentes:>15,} | {sem_correspondencia:>18,}\n".replace(",", ".")

    log_content += separator
    log_content += f"Total de Colunas Adicionadas: {len(pair_map)} | Total de Registros Processados: {total_rows:,}\n".replace(",", ".")
    log_content += separator + "--- DETALHAMENTO DE AGREGAÇÃO ---n"
    ...
    log_content += separator + "Nota: 'TRATADOS' representa a soma de Dados Ausentes e Sem Correspondência que foram higienizados na agregação." + separator
    ...
    ordered_cols = [gk for gk in keys_to_clean.keys() if gk in cols_remaining]
    for key, desc in pair_map.items():
        if key in cols_remaining: ordered_cols.extend([key, desc])
    ...
    con.execute(f"CREATE TABLE work_db.silver_{TABLE_NAME}_final AS SELECT {', '.join(final_ordered_list)} FROM {step3_table}")

    # ------------------------------------------------------------------
    # GRAVAÇÃO E LIMPEZA
    # ------------------------------------------------------------------
    print("💾 Gravando Silver...")
    con.execute(f"COPY (SELECT * FROM work_db.silver_{TABLE_NAME}_final ORDER BY ano_mes) TO '{SILVER_PATH}' (FORMAT PARQUET, PARTITION_BY (ano_mes), OVERWRITE_OR_IGNORE 1)")
    ...
    print(f"🏁 Pipeline {TABLE_NAME} finalizado!")

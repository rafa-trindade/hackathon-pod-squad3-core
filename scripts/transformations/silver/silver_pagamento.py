...
def run():
    ...
    # ------------------------------------------------------------------
    # ETAPA 2: Deduplicação (Otimizada e Segura)
    # ------------------------------------------------------------------
    print("--------------------------------------------------")
    ...
    chave_cols_str = ", ".join([f"t.{c}" for c in chave_tecnica_cols])

    con.execute(f"""
        CREATE TABLE work_db.silver_{TABLE_NAME}_step2 AS
        SELECT t.* FROM {step1_table} t
        QUALIFY ROW_NUMBER() OVER(
            PARTITION BY {chave_cols_str} 
            ORDER BY t.ingestion_ts DESC, t.dat_status_fatura DESC
        ) = 1
    """)
    con.execute("CHECKPOINT work_db")
